...
const loadSettings = () => {
  ...
  return s ? JSON.parse(s) : { lang: 'en', accessibleNav: false, voiceAlerts: true, contactMode: false };
};
...
export default function App() {
  ...
  const handleLogin = (e) => {
    ...
    if (loginEmail.trim() === '' || loginPass.trim() === '') {
      addNotification('Please enter an email and password.');
      return;
    }
    setIsAdmin(false);
    setIsAuthenticated(true);
    ...
    return (
      <div className={`app-container ${!isDarkMode ? 'light-mode' : ''}`}>
        <main className="app-main emergency-view" style={{ justifyContent: 'center', background: 'var(--bg-dark)' }}>
          <h1 style={{ fontSize: '2.5rem', marginBottom: '20px', background: 'linear-gradient(to right, var(--text-main), var(--accent-cyan))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', textAlign: 'center' }}>
            {t.appTitle}
          </h1>
          <form onSubmit={handleLogin} style={{ background: 'var(--bg-translucent)', padding: '25px', borderRadius: '16px', border: '1px solid var(--border-muted)', width: '85%', maxWidth: '350px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
            <h2>{t.signIn}</h2>
            <input type="email" placeholder={t.email} value={loginEmail} onChange={e => setLoginEmail(e.target.value)} style={{ padding: '14px', borderRadius: '8px', border: '1px solid var(--border-muted)', background: 'var(--bg-panel)', color: 'var(--text-main)', fontSize: '1rem' }} />
            <input type="password" placeholder={t.password} value={loginPass} onChange={e => setLoginPass(e.target.value)} style={{ padding: '14px', borderRadius: '8px', border: '1px solid var(--border-muted)', background: 'var(--bg-panel)', color: 'var(--text-main)', fontSize: '1rem' }} />
